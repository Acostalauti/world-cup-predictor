...
def get_stadium_info(
    ...
    for key in ["Name", "StadiumName"]:
        if stadium_dict.get(key):
            ...
            if isinstance(val, list) and len(val) > 0:
                stadium_name = (
                    val[0].get("Description") if isinstance(val[0], dict) else val[0]
                )
            else:
                stadium_name = val
            break
    ...
    if city_info:
        if isinstance(city_info, list) and len(city_info) > 0:
            city_name = (
                city_info[0].get("Description")
                if isinstance(city_info[0], dict)
                else city_info[0]
            )
        else:
            city_name = city_info
...
def normalize_match(match: Dict) -> Dict:
    ...
    if stage and isinstance(stage, list) and len(stage) > 0:
        stage_name = (
            stage[0].get("Description") if isinstance(stage[0], dict) else stage[0]
        )
    ...
    if group and isinstance(group, list) and len(group) > 0:
        group_name = (
            group[0].get("Description") if isinstance(group[0], dict) else group[0]
        )
...
def scrape_fifa_fixture(language: str = "es", max_matches: int = 500) -> pd.DataFrame:
    ...
    """
    matches_dict = scrape_fifa_fixture_dict(language, max_matches)
...
def scrape_fifa_fixture_dict(
    language: str = "es", max_matches: int = 500
) -> list[dict]:
    """
    Obtiene los partidos del Mundial 2026 como lista de diccionarios.
    Útil para integración directa sin pandas.
    ...
    Returns:
        Lista de diccionarios con los fixtures del Mundial 2026
    """
    params = {
        "language": language,
        "count": max_matches,
...
def map_fifa_status(fifa_status: int) -> str:
    """
    Mapea el status de FIFA al status de nuestra app.

    Args:
        fifa_status: Código de status de FIFA (0, 1, 3, etc.)

    Returns:
        Status mapeado: 'upcoming', 'live', 'finished'
    """
    # FIFA status codes (observados):
    # 0 = Finished
    # 1 = Upcoming/Scheduled
    # 2 = Live/In Progress
    # 3 = Finished (alternative)

    if fifa_status in [0, 3]:
        return "finished"
    elif fifa_status == 2:
        return "live"
    else:
...
if __name__ == "__main__":
    ...
    df.to_json(
        "fixture_mundial_2026.json", orient="records", force_ascii=False, indent=2
    )
