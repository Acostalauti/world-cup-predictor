...
def load_fixtures_from_json(json_path: str) -> list[Match]:
    ...
    for fixture in fixtures:
        ...
        # Parse the date from ISO format (keep as datetime, not just date)
        ...
        match = Match(
            ...
            fifaMatchId=fixture.get("match_id"),  # Store FIFA's original ID
        )
...
if __name__ == "__main__":
    ...
    print(f"\nFirst match:")
    print(
        f"  {matches[0].homeTeam} {matches[0].homeFlag} vs {matches[0].awayFlag} {matches[0].awayTeam}"
    )
    ...
    print(f"\nLast match:")
    print(
        f"  {matches[-1].homeTeam} {matches[-1].homeFlag} vs {matches[-1].awayFlag} {matches[-1].awayTeam}"
    )
    ...
    if tbd_matches:
        print(
            f"  Example: Match #{tbd_matches[0].matchNumber} - {tbd_matches[0].homeTeam} vs {tbd_matches[0].awayTeam}"
        )
